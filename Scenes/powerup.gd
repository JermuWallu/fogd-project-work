...
func _ready() -> void:
	...
	# Optional: Add some visual effect like rotation
	create_tween().tween_property(self, "rotation", TAU, 2.0).set_loops()

func _on_body_entered(body: Node2D) -> void:
	"""Handle collision with player"""
	if body.is_in_group("Player"):
		print("Player collected powerup! Press powerup key to reverse gravity.")
		
		# Enable powerup in player
		if body.has_method("enable_powerup"):
			body.enable_powerup()
		
		audio.play()
		
		# Disappear the powerup
		disappear()func disappear():
